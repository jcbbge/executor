...
export interface RuleLoaderConfig {
  ...
  basePath?: string;
  /** Subdirectory for rules. Defaults to "rules" */
  rulesSubdir?: string;
}

const DEFAULT_RULES_SUBDIR = "rules";
 ...
 */
const parseFrontmatter = (content: string): { metadata: RuleMetadata; body: string } => {
  ...
  return {
    metadata: {
      ...
      globs: Array.isArray(metadata.globs) ? metadata.globs.map(String) : undefined,
    },
    body,
  };
};
 ...
 * Does NOT handle nested objects deeply - only 1-2 levels for rule metadata
 */
...
export interface RuleLoader {
  /** Load a specific rule by name (filename without .md extension) */
  load(name: string): Effect.Effect<Rule, Error>;
  /** List all available rules */
  list(): Effect.Effect<RuleMetadata[], Error>;
 ...
 */
export const createRuleLoader = (config?: RuleLoaderConfig): RuleLoader => {
  const basePath = resolveSchemaPath(config?.basePath);
  const rulesPath = join(basePath, config?.rulesSubdir || DEFAULT_RULES_SUBDIR);

  const loadRuleFile = async (filename: string): Promise<Rule> => {
    const ruleFilePath = join(rulesPath, filename);
  ...
  return {
    load: (name) =>
      Effect.tryPromise({
        try: async () => {
          // Rules are flat files: rules/{name}.md
          const filename = name.endsWith(".md") ? name : `${name}.md`;
          return await loadRuleFile(filename);
        },
        catch: (error) => {
          if (error instanceof Error) {
            return new Error(`Failed to load rule "${name}": ${error.message}`);
          }
          return new Error(`Failed to load rule "${name}": ${String(error)}`);
        },
      }),
        ...
        try: async () => {
          ...
          for (const entry of entries) {
            ...
            if (entry.isFile() && entry.name.endsWith(".md") && entry.name !== "README.md") {
              try {
                const rule = await loadRuleFile(entry.name);
          ...
          if (error instanceof Error) {
            return new Error(`Failed to list rules: ${error.message}`);
          }
          return new Error(`Failed to list rules: ${String(error)}`);
        },
      }),
    ...
    exists: (name) =>
      Effect.tryPromise({
        try: async () => {
          try {
            const filename = name.endsWith(".md") ? name : `${name}.md`;
            await loadRuleFile(filename);
            return true;
          } catch {
            return false;
          }
        },
        catch: () => new Error("Failed to check rule existence"),
      }),
  };
};

/**
 * Get the default rule loader (uses env var or default path)
 */
export const getDefaultRuleLoader = (): RuleLoader => {
  return createRuleLoader();
};
