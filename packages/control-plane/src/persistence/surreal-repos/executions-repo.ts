...
export const createExecutionsRepo = (client: SurrealClient) => ({
  getById: (executionId: Execution["id"]) =>
    client.use("rows.executions.get_by_id", async (db) => {
      const result = await db.query<[Array<Record<string, unknown>>]>(
        `SELECT *, meta::id(id) AS id FROM executions WHERE id = type::thing('executions', $id) LIMIT 1`,
        { id: executionId },
      );
      const rows = result[0] ?? [];
  ...
  getByWorkspaceAndId: (
    ...
    client.use("rows.executions.get_by_workspace_and_id", async (db) => {
      const result = await db.query<[Array<Record<string, unknown>>]>(
        `SELECT *, meta::id(id) AS id FROM executions WHERE workspaceId = $workspaceId AND id = type::thing('executions', $id) LIMIT 1`,
  ...
  update: (
    ...
    client.use("rows.executions.update", async (db) => {
      const result = await db.query<[Array<Record<string, unknown>>]>(
        `UPDATE type::thing('executions', $id) MERGE $patch RETURN *, meta::id(id) AS id`,
